...
class DEEPLAB(object):
    ...
    def input_producer(self):
        def read_data():
            image, depth, norm, mask, depth_mask, grid = tf.py_func(myfunc, [self.data_queue[0]],
        ...
        with tf.variable_scope('input'):
            ...
            batch_images, batch_depths, batch_norms, batch_masks, batch_depth_masks, grid = tf.train.batch(
                [images, depths, norms, masks, depth_masks, grid], batch_size=self.batch_size, num_threads=4,
                capacity=60)
        ...
        with slim.arg_scope([slim.conv2d, slim.conv2d_transpose], activation_fn=tf.nn.relu, stride=1,
            ...
            with tf.variable_scope('noise', reuse=reuse):
                ...
                norm_pred_all = tf.concat(
                    [tf.expand_dims(tf.squeeze(fc8_upsample_norm), axis=0), norm_pred_noise, inputs * 0.00392156862],
                    axis=3)

                norm_pred_all = slim.repeat(norm_pred_all, 3, slim.conv2d, 128, [3, 3], rate=2,
                                            weights_initializer=tf.contrib.layers.xavier_initializer(uniform=False),
                                            biases_initializer=tf.constant_initializer(0.0),
                                            scope='conv1_norm_noise_new')
                norm_pred_all = slim.repeat(norm_pred_all, 3, slim.conv2d, 128, [3, 3],
                                            weights_initializer=tf.contrib.layers.xavier_initializer(uniform=False),
                                            biases_initializer=tf.constant_initializer(0.0),
            ...
            with tf.variable_scope('norm_depth', reuse=reuse):
                ...
                valid_angle = tf.multiply(valid_angle, tf.reciprocal(
                    tf.tile(tf.reduce_sum(valid_angle, [3, 4], keep_dims=True) + 1e-5, [1, 1, 1, 81, 1])))
                ...
                depth_all = tf.expand_dims(
                    tf.concat([depth_stage1, exp_depth, tf.squeeze(inputs) * 0.00392156862], axis=2), axis=0)

                depth_pred_all = slim.repeat(depth_all, 3, slim.conv2d, 128, [3, 3], rate=2,
                                             weights_initializer=tf.contrib.layers.xavier_initializer(uniform=False),
                                             biases_initializer=tf.constant_initializer(0.0),
                                             scope='conv1_depth_noise_new')
                depth_pred_all = slim.repeat(depth_pred_all, 3, slim.conv2d, 128, [3, 3],
                                             weights_initializer=tf.contrib.layers.xavier_initializer(uniform=False),
                                             biases_initializer=tf.constant_initializer(0.0),
            ...
            with tf.variable_scope('edge_refinemet', reuse=reuse):
                ...
                edges_encoder = slim.repeat(edge_inputs, 3, slim.conv2d, 32, [3, 3], rate=2,
                                            weights_initializer=tf.contrib.layers.xavier_initializer(uniform=False),
                                            biases_initializer=tf.constant_initializer(0.0),
    ...
    def train(self):
        ...
        for step in range(0, self.max_steps):
            ...
            _, loss_value, loss1_value, loss2_value, loss3_value, loss4_value, global_norm_tmp_value, lr_val, global_step_val = sess.run(
                [train_op, loss, loss1, loss2, loss3, loss4, global_norm_tmp, lr, self.global_step])
            ...
            if step % 10 == 0:
                ...
                format_str = (
                    '%s: step %d, loss = %.5f, loss1 = %.5f, loss2 = %.5f, loss3 = %.5f, loss4 = %.5f lr = %.10f (%.1f examples/sec; %.3f '
                    'sec/batch)')
                print (global_norm_tmp_value)
                print (format_str % (
                datetime.now(), step, loss_value, loss1_value, loss2_value, loss3_value, loss4_value, lr_val,
